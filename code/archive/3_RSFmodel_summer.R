...
# Run it:
dfTT <- test.train.fn(dfRSF.s)

dfRSF.s.t <- as.data.frame(dfTT[[1]])
...
mod.scale.fn <- function(df) {
  ...
  modG[["aic"]] <- aictab(cand.set = modG[1:8], modnames = pred, sort = T)
...
# Run it:
modG.s <- mod.scale.fn(dfRSF.s)

## slope.500 and vrm.500 are best.
save(modG, file = "./Data/Rdata/ModG.s.Rdata", compress = "gzip")
rm(modG)
...
modT[[3]] <- glmer(Response ~ slopeS.500 + (1|CollarID),           # Slope, 500m
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
modT[[4]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + (1|CollarID),           # Slope^2, 500m
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
modT[[5]] <- glmer(Response ~ aspectS.30 + (1|CollarID),          # Aspect, 30m
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
modT[[6]] <- glmer(Response ~ vrmS.500 + (1|CollarID),             # VRM, 500m
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
...
modT[[7]] <- modT[[7]]$bobyqa.  # bobyqa optimizer converged.

modT[[8]] <- glmer(Response ~ elevS.30 + elevS2.30 + slopeS.500 + slopeS2.500 + (1|CollarID),
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
modT[[9]] <- glmer(Response ~ elevS.30 + elevS2.30 + slopeS.500 + slopeS2.500 + vrmS.500 + (1|CollarID),
                   data = dfRSF.s,
                   family = binomial(link = "logit"))
modT[[10]] <- glmer(Response ~ elevS.30 + elevS2.30 + slopeS.500 + slopeS2.500 + vrmS.500 + aspectS.30 + (1|CollarID),
                    data = dfRSF.s,
                    family = binomial(link = "logit"))
...
pred <- c("Elev.30", "Elev^2.30", "Slope.500", "Slope^2.500", "Aspect.30", "vrm.500",
          "(Null)", "Elev^2.30 + Slope^2.500", "Elev^2.30 + Slope^2.500 + vrm.500",
          "Elev^2.30 + Slope^2.500 + vrm.500 + aspect.30")
...
save(modT, file = "./Data/Rdata/ModT.s.Rdata", compress = "gzip")
rm(modT)
...
# Run it:
modG <- mod.scale.fn(dfRSF.s) ; beepr::beep(4)
modG

save(modG,
     file = "./Data/Rdata/ModG.s.rock.Rdata",
     compress = "gzip")
...
modH[[1]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # forest, 1000m
                     aspectS.30 + forest.1000 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[2]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # shrub, 500m
                     aspectS.30 + shrub.500 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[3]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra, 100m
                     aspectS.30 + tundraHeath.100 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[4]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # meadow, 1000m
                     aspectS.30 + meadow.1000 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[5]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # water, 1000m
                     aspectS.30 + water.1000 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[6]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # rock, 1000m
                     aspectS.30 + rock.1000 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[7]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # (null)
                     aspectS.30 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[8]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra + water
                     aspectS.30 + tundraHeath.30 + water.1000 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[9]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra + water + shrub
                     aspectS.30 + tundraHeath.30 + water.1000 + shrub.500 + (1|CollarID),
                   data = dfRSF.s, family = binomial(link = "logit"))
modH[[10]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra + water + shrub + meadow
                      aspectS.30 + tundraHeath.30 + water.1000 + shrub.500 +
                      meadow.1000 + (1|CollarID),
                    data = dfRSF.s, family = binomial(link = "logit"))
modH[[11]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra + water + shrub + meadow + rock
                      aspectS.30 + tundraHeath.30 + water.1000 + shrub.500 +
                      meadow.1000 + rock.500 + (1|CollarID),
                    data = dfRSF.s, family = binomial(link = "logit"))
modH[[12]] <- glmer(Response ~ slopeS.500 + slopeS2.500 + vrmS.500 + elevS.30 + elevS2.30 +   # tundra + water + shrub + meadow + rock + forest
                      aspectS.30 + tundraHeath.30 + water.1000 + shrub.500 +
                      meadow.1000 + rock.500 + forest.500 + (1|CollarID),
                    data = dfRSF.s, family = binomial(link = "logit"))
...
pred <- c("slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Forest.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Shrub.500",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Meadow.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Water.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Rock.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100 + Water.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100 + Water.1000 + Shrub.500",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100 + Water.1000 + Shrub.500 + Meadow.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100 + Water.1000 + Shrub.500 + Meadow.1000 + Rock.1000",
          "slopeS.100 + vrmS.30 + elevS.30 + elevS2.30 + aspectS.30 + Tundra.100 + Water.1000 + Shrub.500 + Meadow.1000 + Rock.1000 + Forest.1000")
...
save(modH, file = "./Data/Rdata/ModH.s.Rdata", compress = "gzip")

# Extract the best model:
best <- modH[[12]]

# Save the best model:
save(best, file = "./Data/Rdata/best.s.Rdata", compress = "gzip")
...
# Add predicted values (based on the best model) to dfRSF.s:
dfRSF.s$pred <- exp(predict(best, newdata=dfRSF.s, re.form=~(1|CollarID), type="link"))

# Evaluate goodness of fit:
library(ResourceSelection)
hoslem.test(dfRSF.s$Response, Pred, 5)
